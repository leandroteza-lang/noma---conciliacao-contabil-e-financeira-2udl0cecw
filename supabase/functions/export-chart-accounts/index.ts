...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (format === 'csv') {
      let csvContent = 'Código Reduzido;Classificação;Nome;Classificação + Nome;Tipo;Empresa\n';
      data.forEach((r: any) => {
        csvContent += `"${r['Código Reduzido'] || ''}";"${r['Classificação'] || ''}";"${r['Nome'] || ''}";"${r['Classificação + Nome'] || ''}";"${r['Tipo'] || ''}";"${r['Empresa'] || ''}"\n`;
      });
    ...
    if (format === 'txt') {
      ...
      data.forEach((r: any) => {
        ...
        txtContent += `Classificação + Nome: ${r['Classificação + Nome'] || '-'}\n`;
    ...
    if (format === 'pdf') {
      ...
      autoTable(doc, {
        startY: 25,
        head: [['Código Reduzido', 'Classificação', 'Nome', 'Classificação + Nome', 'Tipo', 'Empresa']],
        body: body,
        theme: 'grid',
