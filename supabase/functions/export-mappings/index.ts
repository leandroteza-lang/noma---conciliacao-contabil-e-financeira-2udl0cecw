...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (format === 'csv') {
      let csvContent = 'Centro de Custo TGA;Conta Contábil;Status;Observações\n'
      flatData.forEach((r: any) => {
        if (r.isHierarchyHeader) {
          ...
          csvContent += `"${ccStr}";"${caStr}";"${r.Status}";"${r.observations || ''}"\n`
        }
      })
    ...
    if (format === 'txt') {
      ...
      flatData.forEach((r: any) => {
        if (r.isHierarchyHeader) {
          ...
          if (r.observations) txtContent += `Observações: ${r.observations}\n`
          txtContent += '-----------------------------------------\n'
        }
      })
...
<body>
  <div class="container">
    ...
    <table>
      <thead>
        <tr>
          <th style="width: 35%;"><span class="badge-de">DE</span> Centro de Custo TGA</th>
          <th style="width: 35%;"><span class="badge-para">PARA</span> Conta Contábil Vinculada</th>
          <th style="width: 10%; text-align: center;">Status</th>
          <th style="width: 20%; text-align: left;">Observações</th>
        </tr>
      </thead>
      <tbody>
        ${data.map((r: any) => `
