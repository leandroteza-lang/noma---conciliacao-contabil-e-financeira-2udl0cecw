              ...
              for (const log of prevLogs) {
                if (log.changes) {
                  if (log.changes._entity_name) {
                    entityName = log.changes._entity_name.new || log.changes._entity_name.old || log.changes._entity_name;
                    if (entityName) break;
                  }
                  if (log.changes.entity_name) {
                    entityName = log.changes.entity_name.new || log.changes.entity_name.old || log.changes.entity_name;
                    if (entityName) break;
                  }
