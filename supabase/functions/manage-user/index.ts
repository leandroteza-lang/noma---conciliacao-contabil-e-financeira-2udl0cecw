...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { action, email, name, role, cpf, phone, department_id, admin_id, user_id, address, observations, permissions, companies, avatar_url } = await req.json()
    ...
    if (action === 'resend_email') {
      ...
      return new Response(JSON.stringify({ success: true, actionLink }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
      ...
      if (cpf && String(cpf).trim() !== '') {
        ...
        if (existingCpf) {
          return new Response(JSON.stringify({ success: false, error: 'O CPF informado já está cadastrado para outro usuário ativo.' }), { 
            status: 200, 
      ...
      if (error && (error.message.includes('already been registered') || error.message.includes('already exists'))) {
        const { data: foundUserId } = await supabaseAdmin.rpc('get_auth_user_by_email', { p_email: email });
        
        if (foundUserId) {
          const { data: existingProfile } = await supabaseAdmin.from('cadastro_usuarios')
          ...
          if (existingProfile) {
            ...
            if (retry.error) {
              return new Response(JSON.stringify({ success: false, error: retry.error.message }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
            }
        ...
        if (errorMessage === 'Database error saving new user') {
          errorMessage = 'Erro no banco de dados ao salvar o usuário. Verifique se os dados inseridos estão corretos ou se há instabilidade.';
        }
      ...
      if (actionLink && actionUser) {
        ...
        const { data: profile } = await supabaseAdmin.from('cadastro_usuarios').select('id').eq('user_id', actionUser.id).maybeSingle();
        if (profile) {
          ...
          if (companies && Array.isArray(companies) && companies.length > 0) {
            const companyInserts = companies.map((orgId: string) => ({ usuario_id: profile.id, organization_id: orgId }));
          ...
          const details = Object.entries(changes).map(([field, { new: newVal }]: [string, any]) => ({
