  ...
  try {
    ...
    if (format === 'txt') {
      ...
      data.forEach((r: any) => {
        ...
        txtContent += `Tipo Lcto: ${r['Tipo Lcto'] || '-'}\n`
        txtContent += `Operacional: ${r['Operacional'] || '-'}\n`
        txtContent += `Tipo TGA: ${r['Tipo TGA'] || '-'}\n`
        txtContent += '-----------------------------------------\n'
      })
    ...
    if (format === 'pdf') {
      ...
      autoTable(doc, {
        startY: 25,
        head: [['Empresa', 'Código', 'Descrição', 'Tipo Lcto', 'Operacional', 'Tipo TGA']],
        body: body,
        theme: 'grid',
