    ...
    if (type === 'COMPANIES') {
      ...
      for (const [orgId, orgRecords] of recordsByOrg.entries()) {
        ...
        for (const item of orgRecords) {
          ...
          const code = getVal(row, ['CODIGOREDUZIDO', 'CODIGO', 'REDUZIDO', 'CODIGODACONTA', 'CODCONTA', 'COD'])
          const name = getVal(row, ['NOMEDACONTA', 'NOME', 'DESCRICAO', 'CONTA', 'TITULO'])
          const accountType = getVal(row, ['TIPODECONTA', 'TIPO', 'NATUREZA'])
          const classification = getVal(row, ['CLASSIFICACAO', 'CONTACONTABIL', 'MASCARA', 'CODIGOCLASSIFICACAO', 'ESTRUTURA', 'NIVEL'])
