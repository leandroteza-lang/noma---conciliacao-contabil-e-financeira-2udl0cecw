...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (payload.action === 'START_BACKGROUND') {
      ...
      if (payload.type === 'ERP_FINANCIAL_MOVEMENTS' || payload.type === 'MAPPINGS') {
        EdgeRuntime.waitUntil((async () => {
          try {
            ...
            const flushBatch = async (isLast: boolean = false) => {
              if (recordsBatch.length === 0) return;
              const jsonStr = JSON.stringify(recordsBatch);
              const rpcName = payload.type === 'ERP_FINANCIAL_MOVEMENTS' ? 'import_erp_movements_batch' : 'import_mappings_batch';
              const bodyStr = payload.type === 'ERP_FINANCIAL_MOVEMENTS'
                 ? `{"p_org_id":"${payload.organizationId}","p_import_id":"${history.id}","p_records":${jsonStr}}`
                 : `{"p_org_id":"${payload.organizationId}","p_import_id":"${history.id}","p_records":${jsonStr},"p_mode":"${payload.mode || 'UPDATE'}"}`;
              ...
              while (retries > 0) {
                try {
                  ...
                  if (resJson.success === false) throw new Error(resJson.error);
                  
                  totalInserted += resJson.inserted || recordsBatch.length;
                  break;
                } catch (e: any) {
                  retries--;
              ...
              if (!isLast) {
                await supabaseAdmin.from('import_history').update({
                  processed_records: totalRecordsProcessed,
                  status: 'Processing'
                }).eq('id', history.id);
              }
            };
            ...
            if (isCsv) {
                ...
                if (char === '"') {
                  ...
                  if (headers.length === 0) {
                    ...
                    for (let j = 0; j < headers.length; j++) {
                      let val: any = currentRow[j] || '';
                      if (dateCols.includes(headers[j])) val = safeParseDate(val) || '';
                      if (numCols.includes(headers[j])) val = safeParseNum(val) !== null ? safeParseNum(val) : '';
                      rowObj[headers[j]] = val;
                    }
                    recordsBatch.push(rowObj);
                    totalRecords++;
              ...
              const wb = XLSX.read(bytes, { type: 'array', sheets: [payload.sheetName || wb.SheetNames[0]] });
              ...
              for (let i = 0; i < rawRecords.length; i++) {
                const r: any = rawRecords[i];
                const norm: any = {};
                for (const key in r) {
                  const cleanKey = key.trim().replace(/[^A-Z0-9]/gi, '').toUpperCase();
                  let val = r[key];
                  if (dateCols.includes(cleanKey)) val = safeParseDate(val) || '';
                  if (numCols.includes(cleanKey)) val = safeParseNum(val) !== null ? safeParseNum(val) : '';
                  norm[cleanKey] = val;
                }
                recordsBatch.push(norm);
                totalRecordsProcessed++;
        ...
        EdgeRuntime.waitUntil((async () => {
          try {
            const res = await fetch(`${supabaseUrl}/functions/v1/import-data`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${supabaseServiceKey}`
            ...
            if (!res.ok) console.error('Failed to trigger background processing', await res.text())
          } catch (e) {
            console.error('Error triggering background process:', e)
          }
        })())
      }
    ...
    if (payload.fileBase64 || payload.filePath) {
      try {
    ...
    if (type === 'COMPANIES') {
      ...
      for (const [orgId, orgRecords] of recordsByOrg.entries()) {
        ...
        if (toDeleteIds.length > 0) {
          for (let i = 0; i < toDeleteIds.length; i += 500) {
            await supabaseAdmin.from('account_mapping').delete().in('id', toDeleteIds.slice(i, i + 500))
          }
        }
      ...
      for (const [orgId, orgRecords] of recordsByOrg.entries()) {
        ...
        for (let i = 0; i < toInsertFM.length; i += 500) {
          const chunkFM = toInsertFM.slice(i, i + 500)
          const chunkAE = toInsertAE.slice(i, i + 500)
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        const rawDataEmissao = getVal(['DATAEMISSAO', 'EMISSAO'])
        const dataEmissao = safeParseDate(rawDataEmissao)
        const valor = safeParseNum(getVal(['VALOR']))

        toInsert.push({
          organization_id: orgId,
          compensado: String(getVal(['COMPENSADO']) || ''),
          tipo_operacao: String(getVal(['TIPOOPERACAO', 'TIPO']) || ''),
          data_emissao: dataEmissao,
          dt_compens: safeParseDate(getVal(['DTCOMPENS', 'COMPENSACAO'])),
          conta_caixa: String(getVal(['CONTACAIXA']) || ''),
          nome_caixa: String(getVal(['NOMECAIXA']) || ''),
          conta_caixa_destino: String(getVal(['CONTACAIXADESTINO']) || ''),
          forma_pagto: String(getVal(['FORMAPAGTO', 'PAGAMENTO']) || ''),
          c_custo: String(getVal(['CCUSTO', 'CENTROCUSTO']) || ''),
          descricao_c_custo: String(getVal(['DESCRICAOCCUSTO']) || ''),
          valor: valor,
          valor_liquido: safeParseNum(getVal(['VALORLIQUIDO'])),
          n_documento: String(getVal(['NDOCUMENTO', 'DOCUMENTO']) || ''),
          nome_cli_fornec: String(getVal(['NOMECLIFORNEC', 'CLIFORNEC']) || ''),
          historico: String(getVal(['HISTORICO', 'DESCRICAO']) || ''),
          fp: String(getVal(['FP']) || ''),
          n_cheque: String(getVal(['NCHEQUE', 'CHEQUE']) || ''),
          data_vencto: safeParseDate(getVal(['DATAVENCTO', 'VENCTO', 'VENCIMENTO'])),
          nominal_a: String(getVal(['NOMINALA', 'NOMINAL']) || ''),
          emitente_cheque: String(getVal(['EMITENTECHEQUE', 'EMITENTE']) || ''),
          cnpj_cpf: String(getVal(['CNPJCPF', 'CNPJ', 'CPF']) || ''),
          n_extrato: String(getVal(['NEXTRATO', 'EXTRATO']) || ''),
          filial: String(getVal(['FILIAL']) || ''),
          data_canc: safeParseDate(getVal(['DATACANC', 'CANCELAMENTO'])),
          data_estorno: safeParseDate(getVal(['DATAESTORNO', 'ESTORNO'])),
          banco: String(getVal(['BANCO']) || ''),
          c_corrente: String(getVal(['CCORRENTE', 'CONTACORRENTE']) || ''),
          cod_cli_for: String(getVal(['CODCLIFOR']) || ''),
          departamento: String(getVal(['DEPARTAMENTO']) || ''),
          status: 'Pendente',
          _rowNum: rowNum,
        })
      }
      ...
      for (let i = 0; i < toInsert.length; i += 500) {
        const chunk = toInsert.slice(i, i + 500)
        const dbChunk = chunk.map((c: any) => { const { _rowNum, ...rest } = c; return rest; })
        const { error: insErr } = await supabaseAdmin.from('erp_financial_movements').insert(dbChunk)
        if (insErr) {
          console.error(`[ERP_FINANCIAL_MOVEMENTS] Insert error:`, insErr)
          chunk.forEach((c: any) => {
            addError(c._rowNum, `Erro na inserção: ${insErr.message}`, c)
          })
        } else {
    ...
    if (payload.action === 'PROCESS_BACKGROUND') {
      ...
      const nextOffset = (payload.offset || 0) + (payload.limit || 2000)
