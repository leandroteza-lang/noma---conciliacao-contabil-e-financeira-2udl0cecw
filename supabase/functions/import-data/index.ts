    ...
    if (type === 'COMPANIES') {
      ...
      for (const [orgId, orgRecords] of recordsByOrg.entries()) {
        ...
        for (const item of orgRecords) {
          ...
          parsedRows.push({ ...item, strCode, strClass, name, accountType })
        }
        ...
        for (const item of parsedRows) {
          ...
          if (existing) {
            if (existing.is_temp) {
              addError(item.rowNum, `Conta duplicada na própria planilha: O Código Reduzido "${strCode}" ou a Classificação "${strClass}" aparecem mais de uma vez.`, row)
            } else {
              const updateRef = { ...payloadData, id: existing.id }
              toUpdate.push(updateRef)
              processedIds.add(existing.id)
              
              if (sClass) existingByClass.set(sClass, { is_temp: true, ref: updateRef })
              if (sCode) existingByCode.set(sCode, { is_temp: true, ref: updateRef })
            }
          } else {
