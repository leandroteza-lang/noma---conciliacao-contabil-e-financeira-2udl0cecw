  ...
  try {
    ...
    if (type === 'COMPANIES') {
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        if (existing) {
          addError(rowNum, `A Conta Contábil "${contaContabil}" já está cadastrada para esta empresa.`, row)
          continue
        }
        ...
        const { error: insertError } = await supabase
          ...
          .insert({
            ...
            account_number: String(row['NROCONTA'] || row['NUMERO DA CONTA'] || ''),
            classification: String(row['CLASSIFICACAO'] || ''),
            check_digit: String(row['DIGITOCONTA'] || row['DIGITO'] || ''),
