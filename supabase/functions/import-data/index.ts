  ...
  try {
    ...
    if (type === 'COMPANIES') {
      ...
      for (const [orgId, orgRecords] of recordsByOrg.entries()) {
        ...
        const existingMapByClassificacao = new Map(existingContas.map((c: any) => [sanitize(c.classification || c.account_code), c]))
        ...
        for (const item of orgRecords) {
          ...
          const strCode = String(code || '').trim()
          const strClass = String(classification || '').trim()
          
          if (strClass) {
            detectedMasks.add(detectMask(strClass))
          }

          const key = sanitize(strClass || strCode)
