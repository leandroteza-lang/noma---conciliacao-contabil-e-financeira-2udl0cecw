    ...
    if (type === 'COMPANIES') {
      ...
      for (const [orgId, orgRecords] of recordsByOrg.entries()) {
        ...
        for (const item of orgRecords) {
          ...
          const code = getVal(row, ['CODIGOREDUZIDO', 'CODIGO', 'REDUZIDO', 'CODIGODACONTA', 'CODCONTA', 'COD', 'CODIGOCONTA', 'CONTAREDUZIDA'])
          const name = getVal(row, ['NOMEDACONTA', 'NOME', 'DESCRICAO', 'CONTA', 'TITULO', 'NOMECONTA', 'DESCRICAOCONTA'])
          const accountType = getVal(row, ['TIPODECONTA', 'TIPO', 'GRUPOCONTABIL', 'TIPOCONTA'])
          const classification = getVal(row, ['CLASSIFICACAO', 'CONTACONTABIL', 'MASCARA', 'CODIGOCLASSIFICACAO', 'ESTRUTURA', 'NIVEL', 'CLASSIFICACAOCONTA', 'MASCARACONTA'])
          const nature = getVal(row, ['NATUREZA', 'NATUREZACONTA'])
          const accountBehavior = getVal(row, ['COMPORTAMENTO', 'TIPOLANCAMENTO', 'TIPOCOMPORTAMENTO'])
          ...
          parsedRows.push({ ...item, strCode, strClass, name, accountType, nature, accountBehavior })
        }
        ...
        for (const item of parsedRows) {
          ...
          const payloadData = {
            ...
            account_behavior: mapping.account_behavior || (accountBehavior ? String(accountBehavior) : null)
          }

          if (existing) {
            if (existing.is_temp) {
              addError(item.rowNum, `Conta duplicada na própria planilha: A combinação de Código Reduzido "${strCode}" e Classificação "${strClass}" aparece mais de uma vez.`, row)
            } else {
              if (mode !== 'INSERT_ONLY') {
        ...
        if (!simulation) {
          ...
          if (toDeleteIds.length > 0) {
            for (let i = 0; i < toDeleteIds.length; i += 500) {
              await supabase.from('chart_of_accounts').update({ deleted_at: new Date().toISOString(), deleted_by: user.id }).in('id', toDeleteIds.slice(i, i + 500))
            }
          }

          for (let i = 0; i < toInsert.length; i += 500) {
            const { error: insErr } = await supabase.from('chart_of_accounts').insert(toInsert.slice(i, i + 500))
