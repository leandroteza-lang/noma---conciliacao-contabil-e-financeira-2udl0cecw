    ...
    if (type === 'COMPANIES') {
      ...
      for (const [orgId, orgRecords] of recordsByOrg.entries()) {
        ...
        for (const item of orgRecords) {
          ...
          const contaContabil = getVal(row, [
            'CONTACONTABIL',
            'CODIGOREDUZIDO',
            'REDUZIDO',
            'CONTA',
            ...
            if (!isNaN(numVal) && String(data).trim() !== '' && numVal > 10000 && numVal < 100000) {
              ...
              if (parts.length === 3) {
                ...
                const parsed = new Date(
                  `${year}-${parts[1].padStart(2, '0')}-${parts[0].padStart(2, '0')}T12:00:00Z`,
                )
            ...
            if (!isNaN(numVal) && String(data).trim() !== '' && numVal > 10000 && numVal < 100000) {
              ...
              if (isNaN(parsed.getTime()) || year < 1900 || year > 2100) {
                if (!allowIncomplete) {
                  addError(
                    rowNum,
                    'A coluna DATA possui formato inválido ou ano fora do limite.',
                    row,
                  )
                  continue
                }
