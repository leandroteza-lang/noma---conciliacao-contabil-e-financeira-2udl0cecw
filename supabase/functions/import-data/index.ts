    ...
    if (type === 'COMPANIES') {
      ...
      for (const [orgId, orgRecords] of recordsByOrg.entries()) {
        ...
        const existingMapByClassificacao = new Map(
          existingContas.map((c: any) => [
            sanitize((c.account_code || '') + (c.classification || '')),
            c,
          ]),
        )
        ...
        for (const item of orgRecords) {
          ...
          const name =
            ...
            row['CONTA']
          const accountType =
            row['TIPO_CONTA'] || row['TIPO DE CONTA'] || row['TIPO'] || row['NATUREZA']
          const classification =
            row['CLASSIFICACAO'] ||
            row['CONTA_CONTABIL'] ||
            row['CONTA CONTABIL'] ||
            row['MASCARA'] ||
            row['CODIGO_CLASSIFICACAO'] ||
            row['CODIGO CLASSIFICACAO'] ||
            row['ESTRUTURA']
          ...
          if (!allowIncomplete && (!classification || String(classification).trim() === '')) {
            addError(
              item.rowNum,
              'A coluna Classificação está vazia ou a coluna não foi encontrada. Certifique-se de que a planilha contém uma coluna chamada "Classificação".',
              row,
            )
            continue
          }
        ...
        if (!simulation) {
          ...
          if (toDeleteIds.length > 0) {
            for (let i = 0; i < toDeleteIds.length; i += 1000) {
              await supabase
                ...
                .in('id', toDeleteIds.slice(i, i + 1000))
            }
          }
          ...
          for (let i = 0; i < toInsert.length; i += 1000) {
            const { error: insErr } = await supabase
              .from('chart_of_accounts')
              .insert(toInsert.slice(i, i + 1000))
