  ...
  try {
    ...
    if (type === 'COMPANIES') {
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        const rawAccountNumber = String(row['NROCONTA'] || row['NUMERO DA CONTA'] || '').trim()
        const rawCheckDigit = String(row['DIGITOCONTA'] || row['DIGITO'] || '').trim()
      ...
      for (const [orgId, orgRecords] of recordsByOrg.entries()) {
        ...
        const existingMapByClassificacao = new Map(existingContas.map((c: any) => [sanitize((c.account_code || '') + (c.classification || '')), c]))
        
        existingContas.forEach((c: any) => {
          if (c.classification) detectedMasks.add(detectMask(c.classification.trim()))
        })
        ...
        for (const item of orgRecords) {
          const row = item.row
          const code = row['CODIGO_REDUZIDO'] || row['CODIGO REDUZIDO'] || row['CODIGO_CONTA'] || row['CODIGO DA CONTA'] || row['CODIGO'] || row['REDUZIDO']
          const name = row['NOME_CONTA'] || row['NOME DA CONTA'] || row['NOME'] || row['DESCRICAO'] || row['CONTA']
          const accountType = row['TIPO_CONTA'] || row['TIPO DE CONTA'] || row['TIPO'] || row['NATUREZA']
          const classification = row['CLASSIFICACAO'] || row['CONTA_CONTABIL'] || row['CONTA CONTABIL'] || row['MASCARA'] || row['CODIGO_CLASSIFICACAO'] || row['CODIGO CLASSIFICACAO'] || row['ESTRUTURA']

          if (!allowIncomplete && (!code || String(code).trim() === '')) {
            addError(item.rowNum, 'A coluna Código Reduzido está vazia.', row)
            continue
          }
          if (!allowIncomplete && (!name || String(name).trim() === '')) {
            addError(item.rowNum, 'A coluna Nome da Conta está vazia.', row)
            continue
          }
          if (!allowIncomplete && (!classification || String(classification).trim() === '')) {
            addError(item.rowNum, 'A coluna Classificação está vazia ou a coluna não foi encontrada. Certifique-se de que a planilha contém uma coluna chamada "Classificação".', row)
            continue
          }
          ...
          // Aplicar máscara inferida se estiver sem pontos
          ...
          const key = sanitize(strCode + strClass)
          const existing = existingMapByClassificacao.get(key)
          ...
          if (existing) {
            ...
            toUpdate.push({ id: existing.id, ...payloadData })
          } else {
            toInsert.push(payloadData)
          }
        }
