    ...
    if (type === 'COMPANIES') {
      ...
      for (const [orgId, orgRecords] of recordsByOrg.entries()) {
        ...
        for (const item of orgRecords) {
          const row = item.row
          const code = row['CODIGO_REDUZIDO'] || row['CODIGO REDUZIDO'] || row['CODIGO_CONTA'] || row['CODIGO DA CONTA'] || row['CODIGO']
          const name = row['NOME_CONTA'] || row['NOME DA CONTA'] || row['NOME']
          const accountType = row['TIPO_CONTA'] || row['TIPO DE CONTA'] || row['TIPO']
