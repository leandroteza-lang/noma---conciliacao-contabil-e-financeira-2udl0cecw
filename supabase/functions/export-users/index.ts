  ...
  try {
    ...
    if (format === 'excel') {
      const worksheet = XLSX.utils.json_to_sheet(data.map((r: any) => ({
        ...
        'Perfil': r.role === 'admin' ? 'Administrador' : r.role === 'supervisor' ? 'Supervisor' : r.role === 'client_user' ? 'Usuário Cliente' : 'Colaborador',
    ...
    if (format === 'pdf') {
      ...
      const body = data.map((r: any) => [
        String(r.name || '-'),
        String(r.email || '-'),
        r.role === 'admin' ? 'Administrador' : r.role === 'supervisor' ? 'Supervisor' : r.role === 'client_user' ? 'Usuário Cliente' : 'Colaborador',
        String(r.department || '-'),
    ...
    if (format === 'csv') {
      ...
      data.forEach((r: any) => {
        ...
        const status = r.approval_status === 'pending' ? 'Em Aprovação' : (r.status ? 'Ativo' : 'Inativo');
