  ...
  public: {
    Tables: {
      ...
      erp_financial_movements: {
        Row: {
          banco: string | null
          c_corrente: string | null
          c_custo: string | null
          cnpj_cpf: string | null
          cod_cli_for: string | null
          compensado: string | null
          conta_caixa: string | null
          conta_caixa_destino: string | null
          created_at: string | null
          data_canc: string | null
          data_emissao: string | null
          data_estorno: string | null
          data_vencto: string | null
          deleted_at: string | null
          deleted_by: string | null
          deletion_requested_at: string | null
          deletion_requested_by: string | null
          departamento: string | null
          descricao_c_custo: string | null
          dt_compens: string | null
          emitente_cheque: string | null
          filial: string | null
          ...
          n_extrato: string | null
          nome_caixa: string | null
          nome_cli_fornec: string | null
          nominal_a: string | null
          organization_id: string | null
          pending_deletion: boolean | null
          status: string | null
        ...
        Relationships: [
          {
            foreignKeyName: 'erp_financial_movements_mapped_account_id_fkey'
          ...
          {
            foreignKeyName: 'erp_financial_movements_organization_id_fkey'
            columns: ['organization_id']
            isOneToOne: false
            referencedRelation: 'organizations'
            referencedColumns: ['id']
    ...
    Functions: {
      ...
      get_erp_filter_options: { Args: { p_org_id?: string }; Returns: Json }
      import_erp_movements_batch: {
        Args: { p_import_id: string; p_org_id: string; p_records: Json }
        Returns: Json
      }
...
//   table_preferences: jsonb (nullable, default: '{}'::jsonb)
...
//   deleted_at: timestamp with time zone (nullable)
//   deleted_by: uuid (nullable)
// Table: erp_financial_movements
//   id: uuid (not null, default: gen_random_uuid())
//   organization_id: uuid (nullable)
//   compensado: text (nullable)
//   tipo_operacao: text (nullable)
//   data_emissao: date (nullable)
//   dt_compens: date (nullable)
//   conta_caixa: text (nullable)
//   nome_caixa: text (nullable)
//   conta_caixa_destino: text (nullable)
//   forma_pagto: text (nullable)
//   c_custo: text (nullable)
//   descricao_c_custo: text (nullable)
//   valor: numeric (nullable)
//   valor_liquido: numeric (nullable)
//   n_documento: text (nullable)
//   nome_cli_fornec: text (nullable)
//   historico: text (nullable)
//   fp: text (nullable)
//   n_cheque: text (nullable)
//   data_vencto: date (nullable)
//   nominal_a: text (nullable)
//   emitente_cheque: text (nullable)
//   cnpj_cpf: text (nullable)
//   n_extrato: text (nullable)
//   filial: text (nullable)
//   data_canc: date (nullable)
//   data_estorno: date (nullable)
//   banco: text (nullable)
//   c_corrente: text (nullable)
//   cod_cli_for: text (nullable)
//   departamento: text (nullable)
//   mapped_account_id: uuid (nullable)
...
//   errors_list: jsonb (nullable, default: '[]'::jsonb)
//   organization_id: uuid (nullable)
//   mode: character varying (nullable)
//   sheet_name: character varying (nullable)
//   ignored_count: integer (nullable, default: 0)
...
// Table: erp_financial_movements
//   FOREIGN KEY erp_financial_movements_deleted_by_fkey: FOREIGN KEY (deleted_by) REFERENCES auth.users(id) ON DELETE SET NULL
//   FOREIGN KEY erp_financial_movements_deletion_requested_by_fkey: FOREIGN KEY (deletion_requested_by) REFERENCES auth.users(id) ON DELETE SET NULL
//   FOREIGN KEY erp_financial_movements_mapped_account_id_fkey: FOREIGN KEY (mapped_account_id) REFERENCES chart_of_accounts(id) ON DELETE SET NULL
//   FOREIGN KEY erp_financial_movements_organization_id_fkey: FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE
//   PRIMARY KEY erp_financial_movements_pkey: PRIMARY KEY (id)
// Table: financial_movements
...
// Table: import_history
//   FOREIGN KEY import_history_organization_id_fkey: FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE
...
//     USING: (user_id = auth.uid())
// Table: erp_financial_movements
//   Policy "org_erp_financial_movements_delete" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: ((organization_id IN ( SELECT organizations.id    FROM organizations   WHERE (organizations.user_id = auth.uid()))) OR (organization_id IN ( SELECT cuc.organization_id    FROM (cadastro_usuarios_companies cuc      JOIN cadastro_usuarios cu ON ((cuc.usuario_id = cu.id)))   WHERE ((cu.email)::text = (auth.jwt() ->> 'email'::text)))))
//   Policy "org_erp_financial_movements_insert" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: ((organization_id IN ( SELECT organizations.id    FROM organizations   WHERE (organizations.user_id = auth.uid()))) OR (organization_id IN ( SELECT cuc.organization_id    FROM (cadastro_usuarios_companies cuc      JOIN cadastro_usuarios cu ON ((cuc.usuario_id = cu.id)))   WHERE ((cu.email)::text = (auth.jwt() ->> 'email'::text)))))
//   Policy "org_erp_financial_movements_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((organization_id IN ( SELECT organizations.id    FROM organizations   WHERE (organizations.user_id = auth.uid()))) OR (organization_id IN ( SELECT cuc.organization_id    FROM (cadastro_usuarios_companies cuc      JOIN cadastro_usuarios cu ON ((cuc.usuario_id = cu.id)))   WHERE ((cu.email)::text = (auth.jwt() ->> 'email'::text)))))
//   Policy "org_erp_financial_movements_update" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: ((organization_id IN ( SELECT organizations.id    FROM organizations   WHERE (organizations.user_id = auth.uid()))) OR (organization_id IN ( SELECT cuc.organization_id    FROM (cadastro_usuarios_companies cuc      JOIN cadastro_usuarios cu ON ((cuc.usuario_id = cu.id)))   WHERE ((cu.email)::text = (auth.jwt() ->> 'email'::text)))))
// Table: financial_movements
...
//   $function$
//
// FUNCTION get_erp_filter_options(uuid)
//   CREATE OR REPLACE FUNCTION public.get_erp_filter_options(p_org_id uuid DEFAULT NULL::uuid)
//    RETURNS jsonb
//    LANGUAGE plpgsql
//   AS $function$
//   DECLARE
...
//   $function$
//
// FUNCTION import_erp_movements_batch(uuid, uuid, jsonb)
//   CREATE OR REPLACE FUNCTION public.import_erp_movements_batch(p_org_id uuid, p_import_id uuid, p_records jsonb)
//    RETURNS jsonb
//    LANGUAGE plpgsql
//    SECURITY DEFINER
//   AS $function$
...
//   $function$
//
// FUNCTION import_erp_movements_batch_v2(uuid, uuid, jsonb, text)
...
//             IF jsonb_array_length(v_errors) < 15000 THEN
//               v_errors := v_errors || jsonb_build_object(
//                 'row', COALESCE(r->>'_originalIndex', '0')::int,
//                 'error', 'Ignorado: Registro já existente.',
//                 'type', 'Ignorado'
//               );
//             END IF;
//             CONTINUE;
...
//   $function$
//
// FUNCTION import_erp_movements_batch_v3(uuid, uuid, jsonb, text)
...
//     LOOP
//       BEGIN
//         -- Tenta converter valores de forma segura
//         BEGIN
//           v_valor := (NULLIF(btrim(COALESCE(r->>'VALOR', r->>'VALORBRUTO', '')), ''))::numeric;
...
//         BEGIN
//           v_valor_liquido := (NULLIF(btrim(COALESCE(r->>'VALORLIQUIDO', r->>'LIQUIDO', '')), ''))::numeric;
//         EXCEPTION WHEN OTHERS THEN v_valor_liquido := v_valor; END;
//
//         BEGIN
//           v_data_emissao := (COALESCE(NULLIF(btrim(r->>'DATAEMISSAO'), ''), NULLIF(btrim(r->>'EMISSAO'), ''), NULLIF(btrim(r->>'DATA'), '')))::date;
//         EXCEPTION WHEN OTHERS THEN v_data_emissao := NULL; END;
//
//         BEGIN
//           v_dt_compens := (COALESCE(NULLIF(btrim(r->>'DTCOMPENS'), ''), NULLIF(btrim(r->>'COMPENSACAO'), '')))::date;
//         EXCEPTION WHEN OTHERS THEN v_dt_compens := NULL; END;
//
//         BEGIN
//           v_data_vencto := (COALESCE(NULLIF(btrim(r->>'DATAVENCTO'), ''), NULLIF(btrim(r->>'VENCTO'), ''), NULLIF(btrim(r->>'VENCIMENTO'), '')))::date;
//         EXCEPTION WHEN OTHERS THEN v_data_vencto := NULL; END;
//
//         BEGIN
//           v_data_canc := (COALESCE(NULLIF(btrim(r->>'DATACANC'), ''), NULLIF(btrim(r->>'CANCELAMENTO'), '')))::date;
//         EXCEPTION WHEN OTHERS THEN v_data_canc := NULL; END;
//
//         BEGIN
//           v_data_estorno := (COALESCE(NULLIF(btrim(r->>'DATAESTORNO'), ''), NULLIF(btrim(r->>'ESTORNO'), '')))::date;
//         EXCEPTION WHEN OTHERS THEN v_data_estorno := NULL; END;
//
//         v_n_extrato := btrim(COALESCE(r->>'NEXTRATO', r->>'EXTRATO', ''));
//         v_n_documento := btrim(COALESCE(r->>'NDOCUMENTO', r->>'DOCUMENTO', r->>'DOC', ''));
//         v_c_custo := btrim(COALESCE(r->>'CCUSTO', r->>'CENTROCUSTO', r->>'CENTRODECUSTO', ''));
...
//               compensado = COALESCE(NULLIF(btrim(r->>'COMPENSADO'), ''), NULLIF(btrim(r->>'STATUS'), '')),
//               tipo_operacao = COALESCE(NULLIF(btrim(r->>'TIPOOPERACAO'), ''), NULLIF(btrim(r->>'TIPO'), '')),
//               data_emissao = COALESCE(v_data_emissao, data_emissao),
//               dt_compens = COALESCE(v_dt_compens, dt_compens),
//               conta_caixa = COALESCE(NULLIF(btrim(r->>'CONTACAIXA'), ''), NULLIF(btrim(r->>'CONTA'), '')),
//               nome_caixa = COALESCE(NULLIF(btrim(r->>'NOMECAIXA'), ''), NULLIF(btrim(r->>'BANCO'), '')),
//               conta_caixa_destino = NULLIF(btrim(r->>'CONTACAIXADESTINO'), ''),
//               forma_pagto = COALESCE(NULLIF(btrim(r->>'FORMAPAGTO'), ''), NULLIF(btrim(r->>'PAGAMENTO'), '')),
...
//               fp = NULLIF(btrim(r->>'FP'), ''),
//               n_cheque = COALESCE(NULLIF(btrim(r->>'NCHEQUE'), ''), NULLIF(btrim(r->>'CHEQUE'), '')),
//               data_vencto = COALESCE(v_data_vencto, data_vencto),
//               nominal_a = COALESCE(NULLIF(btrim(r->>'NOMINALA'), ''), NULLIF(btrim(r->>'NOMINAL'), '')),
//               emitente_cheque = COALESCE(NULLIF(btrim(r->>'EMITENTECHEQUE'), ''), NULLIF(btrim(r->>'EMITENTE'), '')),
//               cnpj_cpf = COALESCE(NULLIF(btrim(r->>'CNPJCPF'), ''), NULLIF(btrim(r->>'CNPJ'), ''), NULLIF(btrim(r->>'CPF'), '')),
//               filial = NULLIF(btrim(r->>'FILIAL'), ''),
//               data_canc = COALESCE(v_data_canc, data_canc),
//               data_estorno = COALESCE(v_data_estorno, data_estorno),
//               banco = COALESCE(NULLIF(btrim(r->>'BANCO'), ''), NULLIF(btrim(r->>'CODBANCO'), '')),
//               c_corrente = COALESCE(NULLIF(btrim(r->>'CCORRENTE'), ''), NULLIF(btrim(r->>'CONTACORRENTE'), '')),
//               cod_cli_for = COALESCE(NULLIF(btrim(r->>'CODCLIFOR'), ''), NULLIF(btrim(r->>'CODCLIENTE'), '')),
//               departamento = NULLIF(btrim(r->>'DEPARTAMENTO'), '')
...
//   $function$
//
// FUNCTION import_mappings_batch(uuid, uuid, jsonb, text)
//   CREATE OR REPLACE FUNCTION public.import_mappings_batch(p_org_id uuid, p_import_id uuid, p_records jsonb, p_mode text)
//    RETURNS jsonb
//    LANGUAGE plpgsql
//    SECURITY DEFINER
//   AS $function$
...
// Table: erp_financial_movements
//   trg_audit_erp_financial_movements: CREATE TRIGGER trg_audit_erp_financial_movements AFTER INSERT OR DELETE OR UPDATE ON public.erp_financial_movements FOR EACH ROW EXECUTE FUNCTION audit_table_update_trigger()
// Table: organizations
...
// Table: erp_financial_movements
//   CREATE INDEX erp_financial_movements_c_custo_idx ON public.erp_financial_movements USING btree (c_custo)
//   CREATE INDEX erp_financial_movements_conta_caixa_idx ON public.erp_financial_movements USING btree (conta_caixa)
//   CREATE INDEX erp_financial_movements_dedup_idx ON public.erp_financial_movements USING btree (organization_id, btrim(COALESCE(n_extrato, ''::text)), btrim(COALESCE(n_documento, ''::text)), COALESCE(valor, valor_liquido, (0)::numeric), btrim(COALESCE(c_custo, ''::text))) WHERE (deleted_at IS NULL)
//   CREATE INDEX erp_financial_movements_mapped_account_idx ON public.erp_financial_movements USING btree (mapped_account_id)
//   CREATE INDEX erp_financial_movements_org_compens_idx ON public.erp_financial_movements USING btree (organization_id, dt_compens)
//   CREATE INDEX erp_financial_movements_org_data_idx ON public.erp_financial_movements USING btree (organization_id, data_emissao)
//   CREATE INDEX erp_financial_movements_status_idx ON public.erp_financial_movements USING btree (status)
//   CREATE INDEX erp_financial_movements_valor_liquido_idx ON public.erp_financial_movements USING btree (valor_liquido)
