...
export default function EntryList({
  ...
  useEffect(() => {
    if (orgId) fetchEntries()
  }, [orgId, refreshKey, dateFrom, dateTo, status])
  ...
  return (
    <div className="space-y-6 bg-white p-6 rounded-lg border border-slate-200 shadow-sm">
      <div className="flex flex-col md:flex-row gap-4 items-end bg-slate-50 p-4 rounded-md border border-slate-100">
            ...
            {entries.map((e) =>
              editId === e.id ? (
                <TableRow key={e.id} className="bg-slate-50/50">
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      <Button size="icon" variant="ghost" onClick={() => handleSave(e.id)}>
                ...
                <TableRow key={e.id}>
                  ...
                  <TableCell>{formatCur(e.amount)}</TableCell>
