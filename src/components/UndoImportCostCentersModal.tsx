...
export function UndoImportCostCentersModal({ isOpen, onClose, onSuccess, organizations }: any) {
  ...
  const handleRestore = async () => {
    ...
    if (
      !confirm(
        'ATENÇÃO: Restaurar os centros de custo apagará a estrutura atual desta empresa e retornará aos dados do backup selecionado. Continuar?',
      )
    )
      return
    ...
    try {
      ...
      while (hasMore) {
        ...
        await supabase.from('cost_centers').delete().in('id', ids)
        processed += ids.length
        setProgress((p) => ({ ...p, current: processed }))
      }

      backupData.sort((a, b) => (a.code?.length || 0) - (b.code?.length || 0))
      ...
      toast({ title: 'Sucesso', description: 'Centros de Custo restaurados com sucesso.' })
      onSuccess()
      onClose()
    } catch (error: any) {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !isRestoring && !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Restaura os centros de custo a partir de um backup automático criado antes de uma
            importação.
          </DialogDescription>
        </DialogHeader>
